...
import com.ctrlcreate.commlink.service.NearbyOrganizationsService;
...
public class NearbyOrganizationsController {

    private final NearbyOrganizationsService organizationsService;
    private static final String ENDPOINT = APIConstants.URI_ORGANIZATIONS


    @GetMapping(value = ENDPOINT)
    public ResponseEntity<ApiResponse<List<Profile>>> getMyPageData() {
        ApiResponse<List<Profile>> apiResponse = new ApiResponse<>();
        apiResponse.setRequestId(IDUtil.generateRequestId());

        try {
            apiResponse.setData(organizationsService.processNearbyOrganisationRetrieval());
